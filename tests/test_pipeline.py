...
class PipelineTests(unittest.TestCase):
    def test_fixture_pipeline_writes_defensible_manifest(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(stat.S_IMODE((output / "timeline_full.jsonl").stat().st_mode), 0o600)
            event_ids = [json.loads(line)["event_id"] for line in (output / "timeline_full.jsonl").read_text().splitlines()]
    ...
    def test_assisted_profile_writes_reports_without_filtering_events(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            report = json.loads((assisted_output / "assisted_investigation.json").read_text())
            self.assertEqual(report["profile_id"], "persistence_backdoor")
            manifest = json.loads((assisted_output / "run_manifest.json").read_text())
            self.assertEqual(manifest["settings"]["threat_type"], "persistence_backdoor")
            self.assertIn("## Assisted Investigation", (assisted_output / "summary.md").read_text())
    ...
    def test_case_pipeline_preserves_collection_provenance(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = run_case_pipeline([fixture, fixture], output, year=2026, case_name="Regression Case")
            ...
            first_event = json.loads((output / "case_timeline_full.jsonl").read_text().splitlines()[0])
            self.assertTrue(first_event["collection_id"])
            case_events = [json.loads(line) for line in (output / "case_timeline_full.jsonl").read_text().splitlines()]
            ...
            findings = json.loads((output / "case_findings.json").read_text())["findings"]
            self.assertTrue(any(item["title"] == "Duplicate Collection Evidence Detected" for item in findings))
