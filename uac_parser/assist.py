...
def load_profiles() -> dict[str, dict[str, Any]]:
    ...
    if not isinstance(profiles, dict) or not profiles:
        raise InvestigationProfileError(
            "Investigation profiles must contain a non-empty profiles mapping."
        )
    for profile_id, profile in profiles.items():
        if (
            not isinstance(profile, dict)
            or not profile.get("label")
            or not isinstance(profile.get("checklist"), list)
        ):
            raise InvestigationProfileError(
                f"Invalid investigation profile: {profile_id}"
            )
    return profiles
...
def build_assisted_investigation(
    ...
    for finding in findings:
        ...
        matched_mitre = sorted(
            focus_mitre & {value for event in related for value in event.mitre}
        )
        severity_score = {
            "critical": 5,
            "high": 4,
            ...
            "low": 2,
            "informational": 1,
        }.get(str(finding.get("severity")), 0)
        score = (
            severity_score + min(5, len(matched_terms) * 2) + min(3, len(matched_mitre))
        )
        ranked.append(
            {
                ...
                "relevance": "primary"
                if matched_terms or matched_mitre
                else "supporting",
        ...
        checklist.append(
            {
                ...
                "event_ids": [
                    event.event_id for event in matched_events[:20] if event.event_id
                ],
                "matched_terms": sorted(
                    {
                        term
                        for term in terms
                        if any(term in _event_text(event) for event in matched_events)
                    }
                ),
            }
        )
    ...
    return {
        ...
        "observed_focus_signals": [
            {"signal": key, "events": value}
...
def write_assisted_investigation(
    output_dir: Path, report: dict[str, Any], *, prefix: str = ""
) -> None:
    if not report:
        return
    ...
    if primary:
        for item in primary:
            reason = (
                ", ".join(item["matched_focus_terms"] + item["matched_mitre"])
                or "severity/context"
            )
            lines.append(
                f"- **[{str(item['severity']).upper()}] {item['title']}**: {item['summary']} (focus: {reason})"
            )
    else:
        lines.append(
            "- No profile-specific finding was promoted. Review supporting findings and coverage gaps."
        )
    lines.extend(["", "## Analyst Checklist"])
    for item in report["checklist"]:
        marker = "OBSERVED" if item["status"] == "observed" else "REVIEW"
        lines.append(
            f"- **{marker}** - {item['question']} ({item['matched_events']} matching event(s))"
        )
...
def _finding_text(finding: dict[str, Any], related: list[TimelineEvent]) -> str:
    values = [
        finding.get("title", ""),
        finding.get("summary", ""),
        *finding.get("tags", []),
    ...
    return (
        ...
        + " "
        + " ".join(_event_text(event) for event in related)
    )
