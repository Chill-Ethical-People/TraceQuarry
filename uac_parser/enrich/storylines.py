...
def build_storylines(events: list[TimelineEvent]) -> list[dict[str, object]]:
    ...
    for idx, event in enumerate(ordered):
        ...
        if {"download_execute_chain", "cron_modified"} & actions or {
            "chmod_executable",
            "miner_execution",
        } <= actions:
            storylines.append(
...
def _bruteforce_to_access_storylines(
    ...
    for src_ip, failures in by_src.items():
        ...
        for success in successes:
            if not success.timestamp:
                continue
            prior = [
                f for f in failures if f.timestamp and f.timestamp <= success.timestamp
            ]
            if len(prior) < 5:
                continue
            post_events = [
                ...
                for e in ordered
                if e.timestamp
                and e.timestamp >= success.timestamp
                and e.event_action
                in {
                    "sudo_command",
                    "shell_command",
...
def _credential_change_storylines(
    ordered: list[TimelineEvent],
) -> list[dict[str, object]]:
    storylines: list[dict[str, object]] = []
    cred_events = [
