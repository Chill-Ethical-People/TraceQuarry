...
def _add(
    event: TimelineEvent, action: str, severity: str, tags: list[str], mitre: list[str]
) -> None:
...
def _apply_registry_tool_tags(
    ...
    for tool_id, rule in registry_tools.items():
        literals = [
            str(value).lower()
            for value in rule.get("match_literals", [])
            if str(value).strip()
        ]
        if not literals or not any(
            _literal_match(text, literal) for literal in literals
        ):
            continue
        ...
        tags = [
            "tooling",
            f"tool.{tool_id}",
            f"tool_category.{category}",
            f"tool_confidence.{confidence}",
        ]
        _add(
            event,
            action,
...
def _literal_match(text: str, literal: str) -> bool:
    ...
    if re.fullmatch(r"[a-z0-9_.+-]+", literal):
        return bool(
            re.search(rf"(?<![a-z0-9_.-]){re.escape(literal)}(?![a-z0-9_.-])", text)
        )
...
def _execution_text(event: TimelineEvent) -> str:
    ...
    if event.event_category == "execution" or event.event_action in {
        "sudo_command",
        "process_execution",
        "shell_command",
    }:
...
def _looks_like_indicator_search(event: TimelineEvent) -> bool:
    ...
    if not text:
        return False
    if re.search(
        r"\b(grep|egrep|fgrep|rg)\b.*(curl\|wget|bash -i|python\.\*connect|exec\.\*socket|/dev/tcp|nc \|)",
        text,
    ):
        return True
...
def derive_findings(
    ...
    *,
    ...
    finding_policies = {
        ...
        "pam_backdoor_candidate",
        "dangerous_file_capability",
        "suid_file_observed",
        "sgid_file_observed",
        "audit_exec_from_tmp",
        "audit_credential_access",
        "audit_ssh_key_tampering",
        "audit_kernel_module_load",
        "world_writable_sensitive_file",
...
def _bruteforce_campaign_findings(events: list[TimelineEvent]) -> list[dict[str, Any]]:
    findings: list[dict[str, Any]] = []
...
def _lateral_movement_findings(
    ...
    outbound_ssh_cmds = [
        e for e in events if "outbound_ssh_command" in e.detection_names
    ]
    outbound_xfer = [
        e for e in events if "outbound_file_transfer_command" in e.detection_names
    ]
    outbound_conn = [
        e for e in events if "outbound_ssh_connection" in e.detection_names
    ]
    ...
    covered = sorted(
        label for label, kinds in coverage_groups.items() if available & kinds
    )
        ...
        contextual.append(
            {
                ...
                "tags": [
                    ...
                    "known_hosts",
                    "contextual_evidence",
                    "not_lateral_movement_by_itself",
                ],
            }
        )
    if missing:
...
def _account_lifecycle_findings(events: list[TimelineEvent]) -> list[dict[str, Any]]:
    findings: list[dict[str, Any]] = []
    account_events = [
        e
        for e in events
        if e.source_type == "account_diff"
        or e.event_action
        in {
            "user_created",
            "user_deleted",
    ...
    created = [
        ...
        if e.event_action
        in {"account_created_since_backup", "user_created", "password_set_new_account"}
    ]
    deleted = [
        e
        for e in account_events
        if e.event_action in {"account_deleted_since_backup", "user_deleted"}
    ]
    pw_changed = [
        e
        for e in account_events
        if e.event_action in {"password_changed", "account_unlocked"}
    ]
    group_changes = [
        e
        for e in account_events
        if e.event_action in {"group_member_added", "group_member_removed"}
    ]
    parts = []
    if created:
        ...
        findings.append(
            {
                ...
                "summary": "Account changes detected via backup file diffing and log analysis: "
                + "; ".join(parts)
                + ".",
