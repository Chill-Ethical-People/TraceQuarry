...
def parse_epoch(value: str | int | float) -> str | None:
    ...
    if ts <= 0:
        return None
    return to_utc_iso(datetime.fromtimestamp(ts, tz=UTC))
...
def parse_syslog(
    value: str, year: int | None = None, timezone_name: str = "UTC"
) -> str | None:
...
def parse_last_style(
    month: str, day: str, time_value: str, year: int | None, timezone_name: str = "UTC"
) -> str | None:
...
def parse_any(
    ...
    return (
        parse_iso(value)
        or parse_apache(value)
        or parse_syslog(value, year=year, timezone_name=timezone_name)
    )
