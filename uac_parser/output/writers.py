...
CSV_FIELDS = [
    "event_id", "timestamp", "timestamp_raw", "timezone", "timezone_confidence", "timestamp_type",
    "host", "collection_id", "collection_name",
    "collection_input", "collection_host", "source_path", "source_type",
    "parser", "event_category", "event_action", "user", "uid", "gid", "src_ip", "dst_ip", "port",
    "process", "pid", "command", "file_path", "severity", "confidence", "mitre",
    "detection_names", "ttp_flags", "tags", "related_event_ids", "summary", "raw", "extra",
]
...
def write_summary(path: Path, events: list[TimelineEvent], findings: list[dict[str, object]], storylines: list[dict[str, object]]) -> None:
    ...
    lat_findings = [f for f in findings if any(t in (f.get("tags") or []) for t in ["lateral_movement", "negative_finding"])]
    if lat_findings:
    ...
    acct_findings = [f for f in findings if "account_lifecycle" in (f.get("tags") or [])]
    ...
    pw_changes = [e for e in acct_events if e.event_action in {"password_changed", "account_unlocked", "password_set_new_account"}]
    group_changes = [e for e in acct_events if "group_member" in e.event_action]
    if created:
        lines.append(f"  - Accounts created: {', '.join(e.user or '?' for e in created)}")
    if deleted:
        lines.append(f"  - Accounts deleted: {', '.join(e.user or '?' for e in deleted)}")
    if pw_changes:
        lines.append(f"  - Password changes: {', '.join(e.user or '?' for e in pw_changes)}")
    if group_changes:
        for e in group_changes:
            lines.append(f"  - {e.summary}")

    lines.extend(["", "## Brute-Force Campaigns"])
    bf_findings = [f for f in findings if "bruteforce_campaign" in (f.get("tags") or [])]
    if bf_findings:
    ...
    if net_events:
        lines.append(f"- {len(listening)} listening port(s), {len(inbound)} inbound connection(s), {len(outbound)} outbound connection(s)")
        suspicious_net = [e for e in net_events if e.severity in {"medium", "high", "critical"}]
    ...
    if storylines:
        for s in storylines:
            lines.append(f"- **{s.get('title')}** ({s.get('start')} to {s.get('end')}): {s.get('summary')}")
    else:
