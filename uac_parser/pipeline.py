...
from uac_parser.loaders.uac_layout import SourceFile, discover_exclusions, discover_sources
...
from uac_parser.parsers import accounts, auditd, auth, bodyfile, persistence, privilege, ssh, syslog
...
from uac_parser.timeline.engine import assign_event_ids, dedupe_events, filter_window, sort_events
...
def run_case_pipeline(
    ...
    *,
    ...
    case_analyses = [analysis for analysis in analyses if analysis.collection_id not in duplicate_ids]

    full_events = assign_event_ids(dedupe_events(sort_events([
    ...
    case_events = assign_event_ids(dedupe_events(sort_events([
    ...
    findings = derive_findings(
        analysis_events,
        available_source_types={source.source_type for analysis in case_analyses for source in analysis.sources},
    )
    ...
    write_json(output_dir / "case_findings.json", {"findings": findings, "storylines": storylines, "correlations": correlations})
    write_json(output_dir / "case_source_index.json", _case_source_index(
        analyses, start, end, timezone_name, iocs or [], case_name, duplicate_groups, threat_type
    ))
    write_json(output_dir / "case_correlation.json", {
        "case_name": case_name,
        "duplicate_collection_groups": duplicate_groups,
        "correlations": correlations,
    })
    write_ioc_hits_with_prefix(output_dir, "case_ioc_hits", ioc_hits)
    write_summary(output_dir / "case_summary.md", mini_events or full_events, findings, storylines)
    _append_case_summary(output_dir / "case_summary.md", analyses, correlations, case_name, duplicate_groups)
    ...
    if threat_type:
        assisted = build_assisted_investigation(
            ...
            analysis_events,
            findings,
            {source.source_type for analysis in case_analyses for source in analysis.sources},
        )
    ...
    _write_case_manifest(output_dir, analyses, start, end, timezone_name, case_name, duplicate_groups, threat_type)
    _emit_progress(progress_callback, stage="case_complete", completed=len(inputs), total=len(inputs))
...
def _run_collection(
    ...
    *,
    ...
    try:
        ...
        events = _attach_collection(events, collection_id, collection_name, collection_input, collection_host)
        events = enrich_events(events)
        events = assign_event_ids(dedupe_events(sort_events(events)))
        events = correlate_state_events(events)
        ...
        if write_outputs:
            _write_collection_outputs(analysis, start, end, timezone_name, len(iocs), threat_type)
            _write_run_manifest(analysis, input_path, start, end, timezone_name, threat_type)
        return analysis
    finally:
        loaded.cleanup()
...
def _write_collection_outputs(
    ...
    write_json(output_dir / "findings.json", {"findings": analysis.findings, "storylines": analysis.storylines})
    write_json(output_dir / "source_index.json", {
        "root": analysis.root,
    ...
    write_summary(output_dir / "summary.md", analysis.mini_events or analysis.full_events, analysis.findings, analysis.storylines)
    _clear_assisted_outputs(output_dir)
    if threat_type:
        assisted = build_assisted_investigation(
            threat_type,
            _analysis_scope(analysis.full_events, analysis.mini_events, bool(start or end)),
            analysis.findings,
...
def _parse_source(parser, source: SourceFile, host: str, year: int | None, timezone_name: str):
    if source.source_type in {"auth_log", "syslog", "cron", "web_log", "login_history"}:
        return parser(source.path, source.relative, host=host, year=year, timezone_name=timezone_name)
...
def _collection_id(input_path: str | Path, name: str, index: int, used_ids: set[str]) -> str:
    slug = re.sub(r"[^a-zA-Z0-9_.-]+", "-", name.strip().lower()).strip("-._")
    slug = slug or f"collection-{index:02d}"
    digest = sha1(str(Path(input_path).expanduser()).encode("utf-8", "replace")).hexdigest()[:8]
...
def _collection_host(host_override: str, collection_name: str, collection_id: str, events: list[TimelineEvent]) -> str:
    if host_override:
...
def _attach_original_event_ids(events: list[TimelineEvent]) -> list[TimelineEvent]:
    ...
    return [
        replace(event, related_event_ids=[id_map.get(event_id, event_id) for event_id in event.related_event_ids])
        for event in reassigned
    ]


def _analysis_scope(full_events, mini_events, windowed: bool):
    if not windowed:
        return full_events
    untimed_high_signal = [
...
def _append_case_summary(
    ...
    if correlations:
        for correlation in correlations:
            lines.append(f"- **{correlation.get('title')}**: {correlation.get('summary')}")
    else:
...
def write_ioc_hits_with_prefix(out_dir: Path, prefix: str, hits: list[dict[str, object]]) -> None:
...
def build_case_correlations(events: list[TimelineEvent]) -> list[dict[str, object]]:
    correlations: list[dict[str, object]] = []
...
def _collection_set(events: list[TimelineEvent]) -> list[str]:
    return sorted({event.collection_id or event.collection_host or event.host for event in events if event.collection_id or event.collection_host or event.host})
...
def _shared_source_ip_correlations(events: list[TimelineEvent]) -> list[dict[str, object]]:
    output = []
    by_ip: dict[str, list[TimelineEvent]] = {}
    for event in events:
        if not event.src_ip:
            continue
        if event.event_category != "authentication" and "bruteforce" not in ",".join(event.tags):
            continue
        by_ip.setdefault(event.src_ip, []).append(event)
    for src_ip, matches in sorted(by_ip.items(), key=lambda item: len(item[1]), reverse=True):
        collections = _collection_set(matches)
        if len(collections) < 2:
            continue
        failures = [event for event in matches if event.event_action == "ssh_login_failure"]
        successes = [event for event in matches if event.event_action == "ssh_login_success"]
        output.append({
...
def _shared_user_correlations(events: list[TimelineEvent]) -> list[dict[str, object]]:
    output = []
    interesting_actions = {"ssh_login_success", "sudo_command", "password_changed", "user_created", "user_modified", "account_unlocked"}
    ...
    for user, matches in sorted(by_user.items(), key=lambda item: len(item[1]), reverse=True):
        collections = _collection_set(matches)
        if len(collections) < 2:
            continue
        output.append({
...
def _shared_tool_correlations(events: list[TimelineEvent]) -> list[dict[str, object]]:
    tools = ["rclone", "anydesk", "teamviewer", "rustdesk", "screenconnect", "logmein", "chisel", "frp", "ngrok", "cloudflared", "xmrig", "kubectl", "docker", "aws", "gsutil", "azcopy"]
    output = []
    for tool in tools:
        pattern = re.compile(rf"(?<![A-Za-z0-9_.-]){re.escape(tool)}(?![A-Za-z0-9_.-])", re.IGNORECASE)
        matches = [
            event for event in events
            if pattern.search("\n".join(filter(None, [event.command, event.file_path, event.summary, event.raw])))
        ]
        collections = _collection_set(matches)
        if len(collections) < 2:
            continue
        output.append({
            "type": "shared_tooling",
            "severity": "high" if tool in {"rclone", "anydesk", "xmrig", "chisel", "frp", "ngrok"} else "medium",
...
def _shared_path_correlations(events: list[TimelineEvent]) -> list[dict[str, object]]:
    output = []
    by_path: dict[str, list[TimelineEvent]] = {}
    for event in events:
        candidate = event.file_path or ""
        if not candidate:
            match = re.search(r"(/tmp|/var/tmp|/dev/shm|/run)/[^\s;|&]+", " ".join(filter(None, [event.command, event.summary, event.raw])))
            candidate = match.group(0) if match else ""
        if not candidate or not re.search(r"^(/tmp|/var/tmp|/dev/shm|/run|/etc/ssh|/root/.ssh|/home/.+/.ssh)", candidate):
            continue
        by_path.setdefault(candidate, []).append(event)
    for path, matches in sorted(by_path.items(), key=lambda item: len(item[1]), reverse=True):
        collections = _collection_set(matches)
        if len(collections) < 2:
            continue
        output.append({
...
def _cross_host_storyline_correlations(events: list[TimelineEvent]) -> list[dict[str, object]]:
    high_events = [
        event for event in sort_events(events)
...
def _case_correlation_finding(correlation: dict[str, object]) -> dict[str, object]:
    return {
...
def _source_record(source: SourceFile) -> dict[str, object]:
    return {
...
def _input_record(input_path: str | Path) -> dict[str, object]:
    path = Path(input_path).expanduser().resolve()
    record: dict[str, object] = {"path": str(path), "kind": "directory" if path.is_dir() else "archive"}
    if path.is_file():
...
def _collection_fingerprint(sources: list[SourceFile]) -> str:
    ...
    if not records:
        return ""
    payload = "\n".join(f"{relative}|{size}|{digest}" for relative, size, digest in sorted(records))
    return sha256(payload.encode("utf-8", "replace")).hexdigest()


def _duplicate_collection_groups(analyses: list[CollectionAnalysis]) -> list[dict[str, object]]:
    ...
    for fingerprint, members in by_fingerprint.items():
        ...
        groups.append({
            ...
            "duplicate_collection_ids": [member.collection_id for member in members[1:]],
            "collection_inputs": [member.collection_input for member in members],
        })
    return groups


def _duplicate_collection_finding(groups: list[dict[str, object]]) -> dict[str, object] | None:
    if not groups:
        return None
    duplicate_ids = [
        collection_id
...
def _output_records(output_dir: Path, *, exclude: set[str] | None = None) -> list[dict[str, object]]:
    records = []
    for path in sorted(output_dir.iterdir()):
        if not path.is_file() or path.name in (exclude or set()):
            continue
        records.append({"name": path.name, "size": path.stat().st_size, "sha256": _file_sha256(path)})
    return records


def _rules_record() -> dict[str, object]:
    path = registry_path()
    if not path.exists():
...
def _write_run_manifest(
    ...
    write_json(path, {
        ...
        "coverage": {
            ...
            "sources_parsed": sum(source.parser_status == "parsed" for source in analysis.sources),
            "sources_failed": sum(source.parser_status == "error" for source in analysis.sources),
