...
def parse_last_output(path: Path, relative: str, host: str = "", year: int | None = None, timezone_name: str = "UTC") -> list[TimelineEvent]:
    events = []
    failed = "lastb" in relative.lower() or "btmp" in relative.lower() or "failed" in relative.lower()
    for raw in read_text_lines(path):
        ...
        timestamp = parse_last_style(
            match.group("mon"), match.group("day"), match.group("time"),
            int(match.group("year") or year or 0) or None,
            timezone_name,
        )
        events.append(TimelineEvent(
            timestamp=timestamp or "",
            timestamp_raw=" ".join(filter(None, [match.group("mon"), match.group("day"), match.group("time"), match.group("year") or str(year or "")])),
