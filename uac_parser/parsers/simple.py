...
WEB_RE = re.compile(
    r'(?P<src_ip>\S+) \S+ \S+ \[(?P<ts>[^\]]+)\] "(?P<method>\S+) (?P<uri>\S+) [^"]+" (?P<status>\d{3}) (?P<size>\S+)'
)
    ...
    for raw, resolved_year in read_syslog_lines(path, year):
        ...
        events.append(
            TimelineEvent(
                ...
                event_action="cron_execution",
                mitre=["T1053.003"],
                severity="low",
                confidence="medium",
                tags=["cron", "scheduled_task"],
                summary=raw,
                raw=raw,
            )
        )
    ...
    for raw in read_text_lines(path):
        ...
        events.append(
            TimelineEvent(
                ...
                timestamp_raw=pending_ts if pending_ts else str(line_no),
                timezone="UTC",
...
def parse_package_log(path: Path, relative: str, host: str = "") -> list[TimelineEvent]:
    ...
    for raw in read_text_lines(path):
        ...
        events.append(
            TimelineEvent(
                timestamp=ts,
                timestamp_raw=raw[:25],
                timezone="UTC",
                timezone_confidence="assumed",
                timestamp_type="log_time",
                host=host,
                source_path=relative,
                source_type="package_log",
                parser="package_log",
...
def parse_systemd(path: Path, relative: str, host: str = "") -> list[TimelineEvent]:
    ...
    tags = ["systemd"]
    mitre = ["T1543.002"]
    if ("[Service]" in content or "[Timer]" in content) and "ExecStart=" in content:
...
def parse_web_log(
    ...
    for raw in read_text_lines(path):
        ...
        if any(
            x in uri.lower()
            for x in ("cmd=", "shell", "upload", ".php", "../", "%2e%2e")
        ):
            severity = "medium"
        ...
        events.append(
            TimelineEvent(
                ...
                timezone="UTC",
                timezone_confidence="source_or_assumed",
                timestamp_type="log_time",
                host=host,
                source_path=relative,
                source_type="web_log",
