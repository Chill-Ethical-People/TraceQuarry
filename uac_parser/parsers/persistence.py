...
SUSPICIOUS = re.compile(
    r"(curl|wget|nc|ncat|bash\s+-i|/dev/tcp|python\s+-c|perl\s+-e|base64|/tmp/|/var/tmp/|/dev/shm|rclone|xmrig)",
    re.I,
)
...
def parse_cron_file(path: Path, relative: str, host: str = "") -> list[TimelineEvent]:
    ...
    for lineno, raw in enumerate(read_text_lines(path), start=1):
        ...
        if (
            ...
            or text.startswith("#")
            or "=" in text
            and not re.match(r"(@|\S+\s+\S+\s+\S+\s+\S+\s+\S+)", text)
        ):
            continue
        ...
        events.append(
            TimelineEvent(
                ...
                source_type="cron_file",
                parser="persistence",
                event_category="persistence",
                event_action="cron_entry_observed",
                command=text,
                file_path=relative,
                severity=severity,
                confidence="medium",
                ...
                mitre_candidates=["T1053.003"],
                summary=f"Cron entry observed in {relative}:{lineno}",
                raw=raw,
                extra={"line": lineno},
            )
        )
...
def parse_systemd_unit(
    ...
    return [
        TimelineEvent(
            ...
            parser="persistence",
            event_category="persistence",
            event_action="systemd_unit_observed",
            ...
            detection_names=detections,
            ttp_flags=detections,
            mitre_candidates=["T1543.002"],
            summary=f"Systemd unit observed: {relative}",
            raw=content[:8000],
        )
    ]
    ...
    for lineno, raw in enumerate(read_text_lines(path), start=1):
        ...
        events.append(
            TimelineEvent(
                ...
                host=host,
                source_path=relative,
                source_type="shell_profile",
                parser="persistence",
                event_category="persistence",
                event_action="shell_profile_entry_observed",
                command=text,
                file_path=relative,
                severity=severity,
                confidence="medium",
                tags=["shell_profile", "persistence"],
                detection_names=detections,
                ttp_flags=detections,
                mitre_candidates=["T1546.004"],
                summary=f"Shell profile entry observed in {relative}:{lineno}",
                raw=raw,
                extra={"line": lineno},
            )
        )
    ...
    for lineno, raw in enumerate(read_text_lines(path), start=1):
        ...
        events.append(
            TimelineEvent(
                ...
                host=host,
                source_path=relative,
                source_type="ld_preload",
                parser="persistence",
                event_category="persistence",
                event_action="ld_preload_entry_observed",
                file_path=text,
                severity="high",
                confidence="medium",
                tags=["ld_preload", "persistence", "hijack_execution_flow"],
                detection_names=["ld_so_preload_modified"],
                ttp_flags=["ld_so_preload_modified"],
                mitre_candidates=["T1574.006"],
                summary=f"LD_PRELOAD entry observed: {text}",
                raw=raw,
                extra={"line": lineno},
            )
        )
...
def parse_pam_config(path: Path, relative: str, host: str = "") -> list[TimelineEvent]:
    events = []
    risky = re.compile(
        r"(pam_exec|pam_python|pam_permit|pam_unix\.so\s+.*nullok|/tmp/|/dev/shm)", re.I
    )
    for lineno, raw in enumerate(read_text_lines(path), start=1):
        text = raw.strip()
        if not text or text.startswith("#"):
        ...
        events.append(
            TimelineEvent(
                ...
                host=host,
                source_path=relative,
                source_type="pam_config",
                parser="persistence",
                event_category="configuration",
                event_action="pam_config_entry_observed",
                command=text,
                file_path=relative,
                severity=severity,
                confidence="medium",
                ...
                detection_names=detections,
                ttp_flags=detections,
                mitre_candidates=["T1556"] if detections else [],
                summary=f"PAM config entry observed in {relative}:{lineno}",
                raw=raw,
                extra={"line": lineno},
            )
        )
